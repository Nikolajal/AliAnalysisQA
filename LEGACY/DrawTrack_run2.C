...
void DrawTrackPhi(TString datatagname , TString mctagname, Float_t maxy)
{
  histotagname = "cutQuality.Phi_phi";
  minx         = 0.0;
  maxx         = 360.;
...
void DrawTrackPhiOuter(TString datatagname , TString mctagname, Float_t maxy)
{
  histotagname = "cutQuality.PhiOuterTPC_phiOuterTPC";
  minx         = 0.0;
  maxx         = 360.;
  xaxisTitle   = "#varphi (deg)";
  DrawTrack(datatagname, mctagname, maxy);
...
void DrawTrackEta(TString datatagname , TString mctagname, Float_t maxy)
{
  histotagname = "cutQuality.Eta_eta";
  minx         = -1.0;
  maxx         = 1.0;
  xaxisTitle   = "#eta";
  DrawTrack(datatagname, mctagname, maxy);
...
void DrawTrack(TString datatagname , TString mctagname, Float_t maxy)
{
  ...
  TH1F *hevent     = (TH1F*)lindata->FindObject(Form("RsnTaskF0_eventVtx", trigger.Data()));
  ...
  TList *linMC       = (TList*)finMC->Get(Form("RsnOut_f0_MC_2sTOF"));
...
void DrawTrackVz(TString datatagname, TString mctagname1, Float_t maxy)
{
  ...
  TH1F *hhisto        = (TH1F*)lindata->FindObject(Form("RsnTaskF0_eventVtx", trigger.Data()));
  TH1F *hhistoMC      = (TH1F*)linMC->FindObject(Form("RsnTaskF0_eventVtx", mctrigger.Data()));
...
void DrawTrackDCA(TString datatagname, TString mctagname, Float_t maxy)
{
  ...
  TList *lindata    = (TList*)findata->Get(Form("RsnOut_RsnOut_f0", trigger.Data()));
  TH1F *hevent      = (TH1F*)lindata->FindObject(Form("RsnOut_f0_eventVtx", trigger.Data()));
  ...
  TList *linMC        = (TList*)finMC->Get(Form("RsnOut_f0"));
...
void DrawTrackcuts(TString datatagname )
{
 //Data File
  ...
  TList *lindata    = (TList*)findata->Get(Form("RsnOut_f0", trigger.Data()));
